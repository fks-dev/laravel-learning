...
document.addEventListener("DOMContentLoaded", (event) => {
    document.getElementById('searchForm').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch("{{ route('admin.adminMgmt.search') }}", {
                method: 'POST',
                body: formData,
            });
            ...
            data.forEach((result) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="text-center">
                        <a class="btn btn-success edit-btn" href="admin-mgmt/${result.id}/edit">編集</a>

                        <form action="admin-mgmt/${result.id}" method="post" class="d-inline">
                            @csrf
                            @method('delete')
